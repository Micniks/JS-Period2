...
export default class UserFacade {
    ...
    //These variables and methods are only used to fix the dummy database passwords
    static passwordsHashed = false;
    static async hashDatabasePasswords(){
        if(!UserFacade.passwordsHashed){
            UserFacade.users.forEach(async (user) => {
                let unhashedPassword = user.password;
                user.password = await bryptAsync(unhashedPassword);
            })
            UserFacade.passwordsHashed = true;
        }
    }
